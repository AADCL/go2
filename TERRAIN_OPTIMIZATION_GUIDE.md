...
public_map.pcd + traversed_path_map.pcd
  -> 临时二维投影，仅提供地图尺寸、分辨率和原点
  -> 分布式 PMF 候选、鲁棒平面重建、与起点地面高度连续的表面
  -> 按相对地面高度重分类；PGM 和六层共用同一表面
  -> 真实障碍最后覆盖
...
- Patchwork++ 名义传感器高度 `0.51 m`。运行时在雷达周围 `1.50 m` 内对最大
  ...
  启动/恢复需连续 5 帧在 `0.43-0.59 m` 内才允许健康门打开。V2.1.1 对已健康
  运行使用 `health/sensor_height_hysteresis_m: 0.02`，即 `0.41-0.61 m` 才触发高度
  越界关闭，避免腿部起伏或坡道过渡处的小偏差取消目标。关闭后重新使用严格范围。
  点云超时、平面质量、地面支持和频率门限不变；异常仍使导航许可关闭。
...
- 分布式 PMF 候选必须通过鲁棒平面一致性检查，并沿高度连续表面连接到测得的
  起点地面。坡顶/坡脚按需缩小拟合支撑范围；不再限制整条坡相对起点的累计高度。
  大块断开屋面被排除，至少 `0.05 m` 的连续实测台阶边缘被保留。
- 普通障碍相对局部地面高度为 `0.05-1.50 m`，更高点不写入二维障碍。
- 墙边缺少同格地面时，只有具备连续竖直实测点的柱才使用邻域地面参考保留墙体。
  参考要求 `1.0 m` 搜索半径、最近地面不超过 `0.8 m`、至少 8 格且 75% 为平面
  内点；只产生障碍证据，不扩张地面或自由区域。无可靠参考的竖直结构保持未知。
...
- 新地图先生成临时二维投影以统一几何信息，不继承其中的绝对 Z 障碍。最终 PGM
  自由区由重建地面及有效轨迹生成，实测障碍最后覆盖。旧版无地形地图的导出不变。
- revision 2 提交前要求至少 100 个地面格；轨迹样本地面覆盖至少 80%、实际 PGM
  自由比例至少 95%、从起点自由连通比例至少 95%。对角连通不允许穿越障碍角。
  结果写入受 SHA256 保护的 `terrain_quality.yaml`。这些指标不等于机器人 footprint
  的碰撞检查；任一门槛不满足时保留上一版正式地图。
...
旧结果，并返回非零状态。提交阶段先备份旧资产，普通 I/O 错误会回滚；若设备掉电，
可能留下 `.go2_terrain_recovery_*` 及混合版本，但 metadata-last 和 SHA256 校验会
拒绝启动导航。先按恢复目录还原完整资产并校验，再重新导出，不能删除元数据强制
降级，也不要直接删除恢复目录。导出开始和提交前都会复核
...
8 个方向扇区中的至少 2 个；这是机器狗 1 的 39 度前倾 MID360 实测可见范围，
...
部署备份位于工作空间 `backups/` 下带时间戳的目录。回滚 V2.1.0 时停止所有 GO2
launch，恢复对应备份文件或在独立目录检出 `v2.0.1` 并重新编译；不要在仍引用地形包的
工作空间直接删除 `go2_terrain` 或 Patchwork++。程序回滚不会自动恢复地图，必须同时
恢复成套兼容资产。具体备份目录及步骤见 `docs/TERRAIN_EXPORT_REVISION2_20260910.md`。
